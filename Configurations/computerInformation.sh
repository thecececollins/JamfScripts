...
<<ABOUT_THIS_SCRIPT
-----------------------------------------------------------------------
	Originally written by: William Smith
...
INSTRUCTIONS
	...
	4) When an end user calls your Help Desk, the technician can instruct
     ...
     Revised by: Cece Collins
     Revisioin date: 7/30/2019
     Manager of IT
...
##Get MacModel
#runCommand=$( curl -s -k -u "$API_USER:$API_PASS" "$JSS/JSSResource/computers/macaddress/$MACaddy | xpath /computer/hardware/model | awk -F '[>|<]' '{print ($3)}' )
...
##Get Asset Tag
runCommand=$( curl -s -k -u "$API_USER:$API_PASS" "$JSS/JSSResource/computers/macaddress/$MACaddy | xpath /computer/general/asset_tag | awk -F '[>|<]' '{print toupper($3)}' )
...
Feel free to take a screenshot of this information and attach it to your IT Suppport ticket."
...
## Run additional commands #####
	...
	#sudo -u "$currentUser" /usr/bin/open "mailto:yoursupport@emailaddresshere.com?subject=Computer Information ($serialNumber)&body=$displayInfo"
